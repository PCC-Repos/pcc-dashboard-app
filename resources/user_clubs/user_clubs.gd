...
func create_club_api(club_id):
	...
	http_req.request(api_base + 'users/%s/clubs/' % current_user_id, PoolStringArray(["Content-Type: application/json"]), true, HTTPClient.METHOD_POST, to_json(dict))
...
func delete_user_club_api(user_id, club_id):
	...
	add_child(http_req)
	http_req.request(api_base + 'users/%s/clubs/%s/' % [user_id, club_id], PoolStringArray(), true, HTTPClient.METHOD_DELETE)
